...
def conv3x3(in_planes, out_planes, stride=1, groups=1):
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, groups=groups, padding=1, bias=False)
...
class ResNext(nn.Module):
    def __init__(self,
        ...
        self.avgpoool = nn.AdaptiveAvgPool2d((1,1))
        self.fc=nn.Linear(512*block.expansion, vector_dim)
    ...
    def _make_layer(self,
        ...
        if stride != 1 or self.inplanes != planes*block.expansion:
            downsample = nn.Sequential(
                # nn.Conv2d(self.inplanes, planes*block.expansion, kernel_size=3, stride=stride, padding=1),
